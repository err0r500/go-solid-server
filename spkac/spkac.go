...
func notAfter() time.Time {
	return time.Date(2049, 12, 31, 23, 59, 59, 0, time.UTC)
}

func subjectAltName() []int {
	return []int{2, 5, 29, 17}
}
...
func (s spkacHandler) NewSpkac(certName, spkac, webidURI string) ([]byte, string, string, error) {
	newSpkac, err := newSPKACx509(webidURI, certName, spkac)
	if err != nil {
		return nil, "", "", err
	}

	pubKey, err := parse(spkac)
	if err != nil {
		return nil, "", "", err
	}

	rsaPub := pubKey.(*rsa.PublicKey)
	return newSpkac, fmt.Sprintf("%x", rsaPub.N), fmt.Sprintf("%d", rsaPub.E), nil
}

func (spkacHandler) ParseSPKAC(spkacBase64 string) (string, string, error) {
	pubKey, err := parse(spkacBase64)
	if err != nil {
		return "", "", err
	}
	rsaPub := pubKey.(*rsa.PublicKey)
	return fmt.Sprintf("%x", rsaPub.N), fmt.Sprintf("%d", rsaPub.E), nil
}

// parse returns the public key from a KEYGEN's SPKAC request
func parse(spkacBase64 string) (pub interface{}, err error) {
	var info spkacInfo
